...
async function pretraziSerije(trazi, stranica) {
    try {
        // Server-side paginacija: server vraća podatke prema appStranicenje iz konfiguracije
        ...
        // Server već vraća paginirane podatke prema appStranicenje
...
function prikaziSerije(serije) {
    ...
    for (let serija of serije) {
        ...
        let link = "";
        if (jePrijavljen) {
            link = `<a href="/detalji?id=${serija.id}" class="link-naziv-serije">${serija.original_name || serija.name}</a>`;
        } else {
            link = `<span>${serija.original_name || serija.name}</span>`;
        }
...
// Funkcije provjeriJeLiPrijavljen() i provjeriPrijavu() su definirane u funkcije.js
