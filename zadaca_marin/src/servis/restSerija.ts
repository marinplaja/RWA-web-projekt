...
export class RestSerija {
	...
	private parsirajId(zahtjev: Request, odgovor: Response): number | null {
	...
	async postSerije(zahtjev: Request, odgovor: Response): Promise<void> {
	...
	async getSerija(zahtjev: Request, odgovor: Response): Promise<void> {
		odgovor.type("application/json");
	...
	async getSerijaOdgledano(zahtjev: Request, odgovor: Response): Promise<void> {
		odgovor.type("application/json");
		const sesija = zahtjev.session as RWASession;
	...
	async postSerijaOdgledano(zahtjev: Request, odgovor: Response): Promise<void> {
		if (!this.provjeriAutorizaciju(zahtjev, odgovor)) return;
		...
		if (!korisnikPodaci) return;

		if (!(await this.provjeriSeriju(id, odgovor))) return;
		if (await this.provjeriJeLiOdgledana(korisnikPodaci.idKORISNIK, id, odgovor)) return;

		await this.dodajOdgledanuSeriju(zahtjev, korisnikPodaci.idKORISNIK, id, odgovor);
	}

	async deleteSerijaOdgledano(zahtjev: Request, odgovor: Response): Promise<void> {
		if (!this.provjeriAutorizaciju(zahtjev, odgovor)) return;
	...
	async getSerijaDetaljiTMDB(zahtjev: Request, odgovor: Response): Promise<void> {
		if (!this.provjeriAutorizaciju(zahtjev, odgovor)) return;
	...
	private async dohvatiDetaljeSaSerijom(id: number) {
		...
		const youtubeVideo = videi.find((v: VideoTmdb) => 
	...
	private async dohvatiKorisnika(zahtjev: Request, odgovor: Response) {
		const sesija = zahtjev.session as RWASession;
	...
	private async provjeriSeriju(id: number, odgovor: Response): Promise<boolean> {
		const serija = await this.sdao.daj(id);
		if (!serija) {
			odgovor.status(404).json({ pogreska: "nema resursa" });
			return false;
	...
	private async provjeriJeLiOdgledana(
		korisnikId: number,
		serijaId: number,
		odgovor: Response
	): Promise<boolean> {
	...
	private async dodajOdgledanuSeriju(
		zahtjev: Request,
		korisnikId: number,
		serijaId: number,
		odgovor: Response
	...
	private async obrisiOdgledanuSeriju(
		zahtjev: Request,
		korisnikId: number,
		serijaId: number,
		odgovor: Response
