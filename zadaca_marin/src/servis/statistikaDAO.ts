...
export interface StatistikaUnos {
	tipRadnje: string;
	stranica?: string | undefined;
	endpoint?: string | undefined;
	vrstaRadnje?: string | undefined;
	korisnik_id?: number | undefined;
	datumVrijeme: string;
}
...
export class StatistikaDAO {
	...
	async dodaj(unos: StatistikaUnos) {
	...
	async dajStatistiku(datumOd: string, datumDo: string): Promise<any> {
		let sqlPrijave = `
		...
		let pristupiPoStranici: { [key: string]: number } = {};
		...
		let pristupiPoEndpointu: { [key: string]: number } = {};
		...
		let radnjeRezultat: { [key: string]: number } = {
			pregled: 0,
			dodavanje: 0,
			azuriranje: 0,
			brisanje: 0,
