...
export class Konfiguracija {
	...
	private pretvoriCSVkonfig(podaci: string) {
		const konf: { [kljuc: string]: string } = {};
		const nizPodataka = podaci.split("\n");
		
		for (const podatak of nizPodataka) {
			const podatakNiz = podatak.split(":");
			if (podatakNiz.length < 2) continue;
	...
	private provjeriTajniKljucSesija() {
		...
		if (kljuc.length < 75 || kljuc.length > 100) {
			throw new Error(
				`tajniKljucSesija mora biti između 75 i 100 znakova. Trenutna duljina: ${kljuc.length}`
			);
		}
	}

	private provjeriAppStranicenje() {
		...
		if (vrijednost < 5 || vrijednost > 20) {
			throw new Error(
				`appStranicenje mora biti broj između 5 i 20. Trenutna vrijednost: ${vrijednost}`
			);
		}
	}

	private provjeriTrajanjeSesije() {
		...
		if (vrijednost < 5 || vrijednost > 30) {
			throw new Error(
				`trajanjeSesije mora biti broj između 5 i 30. Trenutna vrijednost: ${vrijednost}`
			);
		}
	}

	private provjeriNeuspjesnePrijave() {
		...
		if (vrijednost < 3 || vrijednost > 10) {
			throw new Error(
				`neuspjesnePrijave mora biti broj između 3 i 10. Trenutna vrijednost: ${vrijednost}`
			);
		}
	}

	private provjeriTmdbApiKey() {
